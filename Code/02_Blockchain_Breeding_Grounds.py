...
for index, row in office_list.iterrows():
    ...
    office_continents[long]=continent
...
nbg_values_p1 = list(df_nbg["2009-2020"])[:10]

nbg_countries = list(df_nbg.index)[:10]

df = pd.DataFrame(data=zip(nbg_countries, nbg_values_p1))
df.rename(columns={0: "Patent Authority", 1: "NBG index"}, inplace=True)
df.sort_values(by=["NBG index"], inplace=True, ascending=True)
continents = []
for index, row in df.iterrows():
    authority = row["Patent Authority"]
    if authority in office_continents:
        continents.append(office_continents[authority])
    else:
        print("Error")
df["Regions"] = continents
...
for index, row in df.iterrows():
    ...
    if region == "Asia":
        ax1.scatter(x = value, y=authority, marker = "o", color="black", s=marker_size)
    elif region == "Australia":
        ax1.scatter(x = value, y=authority, marker = "X", color="black", s=marker_size)
    elif region == "Europe":
        ax1.scatter(x = value, y=authority, marker = "s", color="black", s=marker_size)
    elif region == "North America":
        ax1.scatter(x = value, y=authority, marker = "o", color="grey", s=marker_size)
    elif region == "South America":
        ax1.scatter(x = value, y=authority, marker = "X", color="grey", s=marker_size)

ax1.scatter(x = -100, y=authority, marker = "o", color="black", s=marker_size, label = "Asia")
ax1.scatter(x = -100, y=authority, marker = "X", color="black", s=marker_size, label = "Australia")
ax1.scatter(x = -100, y=authority, marker = "s", color="black", s=marker_size, label = "Europe")
ax1.scatter(x = -100, y=authority, marker = "o", color="grey", s=marker_size, label = "North America")
#ax1.scatter(x = -100, y=authority, marker = "X", color="grey", s=marker_size, label = "South America")
...
ax2.set_xlabel("NBG weighted index", style="italic")
ax1.set_ylabel(None)


df_wnbg.sort_values(by=["2009-2020"], inplace=True, ascending=False)

wnbg_values_p1 = list(df_wnbg["2009-2020"])[:10]

wnbg_countries = list(df_wnbg.index)[:10]

df = pd.DataFrame(data=zip(wnbg_countries, wnbg_values_p1))
df.rename(columns={0: "Patent Authority", 1: "NBG weighted index"}, inplace=True)
df.sort_values(by=["NBG weighted index"], inplace=True, ascending=True)
...
#sns.scatterplot(ax=ax2, data=df, x="NBG weighted index", y="Patent Authority", hue="Regions", hue_order=["Asia", "Australia", "Europe", "North America"])


for index, row in df.iterrows():
    authority = row["Patent Authority"]
    value = row["NBG weighted index"]
    region = row["Regions"]
    if region == "Asia":
        ax2.scatter(x = value, y=authority, marker = "o", color="black", s=marker_size)
    elif region == "Australia":
        ax2.scatter(x = value, y=authority, marker = "X", color="black", s=marker_size)
    elif region == "Europe":
        ax2.scatter(x = value, y=authority, marker = "s", color="black", s=marker_size)
    elif region == "North America":
        ax2.scatter(x = value, y=authority, marker = "o", color="grey", s=marker_size)
    elif region == "South America":
        ax2.scatter(x = value, y=authority, marker = "X", color="grey", s=marker_size)
ax2.scatter(x = -100, y=authority, marker = "o", color="black", s=marker_size, label = "Asia")
#ax2.scatter(x = -100, y=authority, marker = "X", color="black", s=marker_size, label = "Australia")
ax2.scatter(x = -100, y=authority, marker = "s", color="black", s=marker_size, label = "Europe")
ax2.scatter(x = -100, y=authority, marker = "o", color="grey", s=marker_size, label = "North America")
#ax2.scatter(x = -100, y=authority, marker = "D", color="black", s=marker_size, label = "South America")
...
ax1.legend(loc='lower right', handlelength=1, fontsize="small")
ax2.legend(loc='lower right', handlelength=1, fontsize="small")
ax2.set_ylabel(None)
fig.tight_layout()
fig.savefig("National Blockchain Breeding Grounds.png", dpi=1600)
plt.show()
...
ax1.grid(color='lightgray', linewidth=0.5, zorder=-1.0)
ax2.grid(color='lightgray', linewidth=0.5, zorder=-1.0)
ibg_values_p1 = list(df_ibg["2009-2020"])[:10]
ibg_countries = list(df_ibg.index)[:10]

df = pd.DataFrame(data=[ibg_countries, ibg_values_p1]).T
df.rename(columns={0: "Patent Authority", 1: "IBG index"}, inplace=True)
df.sort_values(by=["IBG index"], inplace=True, ascending=True)
...
ax1.scatter(x = -100, y=authority, marker = "o", color="black", s=marker_size, label = "Asia")
#ax1.scatter(x = -100, y=authority, marker = "X", color="black", s=marker_size, label = "Eurasia")
ax1.scatter(x = -100, y=authority, marker = "s", color="black", s=marker_size, label = "Europe")
ax1.scatter(x = -100, y=authority, marker = "o", color="grey", s=marker_size, label = "North America")
#ax1.scatter(x = -100, y=authority, marker = "X", color="grey", s=marker_size, label = "South America")
...
ax2.set_xlabel("IBG weighted index", style="italic")
df_wibg.sort_values(by=["2009-2020"], inplace=True, ascending=False)

wibg_values_p1 = list(df_wibg["2009-2020"])[:10]
wibg_countries = list(df_wibg.index)[:10]

df = pd.DataFrame(data=[wibg_countries, wibg_values_p1]).T
df.rename(columns={0: "Patent Authority", 1: "IBG weighted index"}, inplace=True)
df.sort_values(by=["IBG weighted index"], inplace=True, ascending=True)
...
ax2.scatter(x = -100, y=authority, marker = "o", color="black", s=marker_size, label = "Asia")
ax2.scatter(x = -100, y=authority, marker = "X", color="black", s=marker_size, label = "Eurasia")
ax2.scatter(x = -100, y=authority, marker = "s", color="black", s=marker_size, label = "Europe")
ax2.scatter(x = -100, y=authority, marker = "o", color="grey", s=marker_size, label = "North America")
ax2.scatter(x = -100, y=authority, marker = "X", color="grey", s=marker_size, label = "South America")
...
fig.tight_layout()
fig.savefig("International Blockchain Breeding Grounds.png", dpi=1600)
plt.show()